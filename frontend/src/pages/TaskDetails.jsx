...
import { addComment, updateTask, deleteTask } from "../features/workspaceSlice";
    ...
    const handleAddComment = async () => {
        ...
        try {
            ...
            if (data?.comment) {
                ...
                dispatch(addComment({ projectId, taskId, comment: data.comment }));
            }
            setNewComment("");
            toast.dismiss(toastId);
                ...
                <div className="p-5 rounded-md  border border-gray-300 dark:border-zinc-800  flex flex-col lg:h-[80vh]">
                        ...
                        {comments.length > 0 ? (
                            <div className="flex flex-col gap-4 mb-6 mr-2">
                                {comments.map((comment) => {
                                    const isOwnComment = comment.userId === user?.id || comment.user?.id === user?.id;
                                    return (
                                        <div key={comment.id} className={`sm:max-w-4/5 dark:bg-gradient-to-br dark:from-zinc-800 dark:to-zinc-900 border border-gray-300 dark:border-zinc-700 p-3 rounded-md ${isOwnComment ? "ml-auto" : "mr-auto"}`} >
                                            <div className="flex items-center gap-2 mb-1 text-sm text-gray-500 dark:text-zinc-400">
                                                <img src={comment.user?.image || assets.profile_img_a} alt="avatar" className="size-5 rounded-full object-cover" />
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-end gap-3">
                        <textarea
                            ...
                            rows={3}
                        />
                        <button onClick={handleAddComment} className="bg-gradient-to-l from-blue-500 to-blue-600 hover:opacity-90 transition-opacity text-white text-sm px-5 py-2 rounded cursor-pointer" >
                            Post
                        </button>
                    </div>
                </div>
            ...
            <div className="w-full lg:w-1/2 flex flex-col gap-6">
                ...
                <div className="p-5 rounded-md bg-white dark:bg-zinc-900 border border-gray-300 dark:border-zinc-800 ">
                    <div className="mb-3">
                        <div className="flex items-start justify-between gap-3 mb-2">
                            ...
                            <select
                                ...
                                className="text-xs px-2 py-1 rounded bg-zinc-100 dark:bg-zinc-800 border border-zinc-300 dark:border-zinc-700 text-zinc-900 dark:text-zinc-200 cursor-pointer focus:outline-none"
                            >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-gray-700 dark:text-zinc-300">
                        <div className="flex items-center gap-2">
                            <img src={task.assignee?.image || assets.profile_img_a} className="size-5 rounded-full object-cover" alt="avatar" />
                            <span>{task.assignee?.name || task.assignee?.email || "Unassigned"}</span>
                        </div>
                        <div className="flex items-center gap-2">
                            <CalendarIcon className="size-4 text-gray-500 dark:text-zinc-500" />
                            <span>Due : {task.due_date ? format(new Date(task.due_date), "dd MMM yyyy") : "Not set"}</span>
                        </div>
                    </div>
                </div>
                ...
                {project && (
                    <div className="p-4 rounded-md bg-white dark:bg-zinc-900 text-zinc-700 dark:text-zinc-200 border border-gray-300 dark:border-zinc-800 ">
                        ...
                        <p className="text-xs mt-3">Project Start Date: {project.start_date ? format(new Date(project.start_date), "dd MMM yyyy") : "Not set"}</p>
