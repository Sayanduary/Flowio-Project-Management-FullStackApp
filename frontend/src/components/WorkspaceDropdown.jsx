        ...
        <div className="relative m-4" ref={dropdownRef}>
            <button onClick={() => setIsOpen(prev => !prev)} className="w-full flex items-center justify-between p-3 h-auto text-left rounded hover:bg-gray-100 dark:hover:bg-zinc-800 cursor-pointer" >
                <div className="flex items-center gap-3">
                    <img src={currentWorkspace?.image_url || "https://avatar.vercel.sh/" + (currentWorkspace?.name || "workspace")} alt={currentWorkspace?.name} className="w-8 h-8 rounded shadow object-cover" />
                    ...
                    <div className="p-2">
                        ...
                        {workspaceList.map((ws) => (
                            <div key={ws.id} onClick={() => onSelectWorkspace(ws.id)} className="flex items-center gap-3 p-2 cursor-pointer rounded hover:bg-gray-100 dark:hover:bg-zinc-800" >
                                <img src={ws.image_url || "https://avatar.vercel.sh/" + ws.name} alt={ws.name} className="w-6 h-6 rounded object-cover" />
                                <div className="flex-1 min-w-0">
                                    <p className="text-sm font-medium text-gray-800 dark:text-white truncate">
                                        {ws.name}
                                    </p>
                                    <p className="text-xs text-gray-500 dark:text-zinc-400 truncate">
                                        {ws.members?.length ?? ws.membersCount ?? 0} members
                                    </p>
                                </div>
